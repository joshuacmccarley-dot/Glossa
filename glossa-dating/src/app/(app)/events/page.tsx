...
import { Calendar, MapPin, Users, Plus, Clock, ArrowUpDown } from "lucide-react";
...
export default function EventsPage() {
  ...
  // Create form
  const [form, setForm] = useState({ title: "", description: "", category: "hangout", location_name: "", starts_at: "", max_attendees: "" });
  ...
  const load = async () => {
    ...
    const { data } = await supabase.from("events").select("*, event_rsvps(user_id, status)").gte("starts_at", now).order("starts_at");

    const enriched = await Promise.all((data || []).map(async (e) => {
      const { data: creator } = await supabase.from("profiles").select("display_name, photos, user_id").eq("user_id", e.creator_id).single();
      ...
      return {
        ...
        distance_miles: (profile?.latitude && e.latitude) ? distanceMiles(profile.latitude, profile.longitude!, e.latitude, e.longitude!) : undefined,
      };
    }));
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-5">
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={() => setCreating(true)} className="flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-orange-400 text-white text-xs font-bold px-4 py-2.5 rounded-full shadow">
      ...
      {creating && (
        <div className="bg-white rounded-2xl border border-amber-200 shadow-md p-5 mb-5 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={createEvent} disabled={formLoading || !form.title || !form.starts_at}
      ...
      <div className="flex gap-2 mb-3 overflow-x-auto pb-1 scrollbar-hide">
        <button onClick={() => setActiveCategory("all")} className={`flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-bold ${activeCategory === "all" ? "bg-amber-500 text-white" : "bg-gray-100 text-gray-600"}`}>All</button>
        {EVENT_CATEGORIES.map((c) => (
          <button key={c.id} onClick={() => setActiveCategory(c.id)} className={`flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-bold ${activeCategory === c.id ? "bg-amber-500 text-white" : "bg-gray-100 text-gray-600"}`}>
            {c.emoji} {c.label}
          </button>
        ))}
      </div>
      <div className="flex gap-2 mb-5">
        {DISTANCE_PRESETS.map((d) => (
          <button key={d.label} onClick={() => setDistanceMax(d.value)} className={`flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-semibold ${distanceMax === d.value ? "bg-amber-500 text-white" : "bg-gray-100 text-gray-500"}`}>{d.label}</button>
        ))}
        <button onClick={() => setSortDir((s) => s === "soonest" ? "closest" : "soonest")} className="flex items-center gap-1 ml-auto bg-gray-100 px-3 py-1.5 rounded-xl text-xs font-semibold text-gray-600">
          <ArrowUpDown className="w-3 h-3" />{sortDir === "soonest" ? "Soonest" : "Closest"}
        </button>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          ...
          <button onClick={() => setCreating(true)} className="bg-gradient-to-r from-amber-500 to-orange-400 text-white px-6 py-3 rounded-full font-semibold text-sm">
            Create an event
          </button>
        </div>
      ) : (
            ...
            return (
              <div key={evt.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="bg-gradient-to-r from-amber-50 to-orange-50 px-4 py-3 flex items-center gap-2">
                  ...
                  {isCreator && <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">Yours</span>}
                </div>
                <div className="px-4 py-3 space-y-2">
                  {evt.description && <p className="text-sm text-gray-600 line-clamp-2">{evt.description}</p>}
                  <div className="flex flex-wrap gap-3 text-xs text-gray-500">
                    <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />{format(new Date(evt.starts_at), "MMM d, h:mm a")}</span>
                    {evt.location_name && <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5" />{evt.location_name}</span>}
                    {evt.distance_miles !== undefined && <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{formatDistance(evt.distance_miles)}</span>}
                    <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" />{evt.rsvp_count ?? 0} going{evt.max_attendees ? ` / ${evt.max_attendees}` : ""}</span>
                  </div>
                  {!isCreator && (
                    <div className="flex gap-2 pt-1">
                      <button
                        onClick={() => rsvp(evt.id, userGoing ? "not_going" : "going")}
                        disabled={isFull && !userGoing}
                        className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition ${userGoing ? "bg-amber-500 text-white" : isFull ? "bg-gray-100 text-gray-400" : "bg-amber-50 text-amber-700 border border-amber-200 hover:bg-amber-100"}`}
                      >
                        {userGoing ? "✓ Going!" : isFull ? "Full" : "I'm going"}
                      </button>
                      <button onClick={() => rsvp(evt.id, "interested")} className="px-4 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50">
                        Interested
                      </button>
                    </div>
                  )}
